    ...
    return {
      ...
      onShow: function () {
        ...
        if (command === 'tableProperties') {
          ...
          // Save a reference to the selected table, and push a new set of
          // default values.
      ...
      onOk: function () {
        ...
        let tablewrapper;
        if (!table.getParent().hasClass("usa-table-container")) {
          tablewrapper = makeElement('div');
          tablewrapper.addClass('usa-table-container');
        }
        else {
          tablewrapper = table.getParent();
        }
        ...
        // There should be a table. So let us add our classes.
        ...
        if (info['tableScrollable']) {
          ...
          if (table.getParent().hasClass('usa-table-container--scrollable')) {
            table.getParent().removeClass('usa-table-container--scrollable');
          }
        }
        ...
        // Check if sortable
        if (info['tableSortable']) {
          // Create announcement div and append to wrapping div.
          let announcement = CKEDITOR.dom.element.createFromHtml('<div class="usa-sr-only usa-table__announcement-region" aria-live="polite"></div>');
          tablewrapper.append(announcement);

          // Add custom sortable table class
          table.addClass('usa-sortable-ckeditor');

          let headers = table.find('th');
          for (let i = 0; i < headers.count(); i++) {
            headers.getItem(i).setAttribute('data-sortable', '');
            headers.getItem(i).setAttribute('role', 'columnheader');
          }
        }
        else {
          if (table.hasClass('usa-sortable-ckeditor')) {
            table.removeClass('usa-sortable-ckeditor');

            let children = tablewrapper.getChildren();
            for (let i = 0; i < children.count(); i++) {
              if (children.getItem(i).hasClass('usa-table__announcement-region')) {
            ...
            let headers = table.find('th');
            for (let i = 0; i < headers.count(); i++) {
              headers.getItem(i).removeAttribute('data-sortable');
              headers.getItem(i).removeAttribute('role');
            }
          }
        }
            ...
            children: [{
              ...
              children: [
                ...
                },
                {
                  ...
                  'default': '',
                  setup: function (selectedTable) {
                    let val = selectedTable.hasClass('usa-sortable-ckeditor');
                    this.setValue(val);
                  },
                  commit: commitValue
                }]
